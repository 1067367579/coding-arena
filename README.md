...
| 指标 | 当前水平 | 优化目标 | 达成方案 |
|------|---------|---------|---------|
| 代码提交QPS | ~50 | 500+ | RabbitMQ异步削峰 |
| 评测响应延迟 | 500ms-2s | <500ms | Docker容器池复用 |
| 热榜查询P99 | <50ms | <20ms | Redis ZSet缓存 |
| 竞赛排名更新 | 实时 | 实时 | Redis List缓存 |
| 网关吞吐量 | 1000 QPS | 5000 QPS | WebFlux非阻塞IO |
